...
#retorna as colunas cidade, latitude, longitudee temperatura máxima
response = supabase.table('clima_cidades_sul').select(
    "cidade, latitude, longitude, temperatura_max"
).eq("dia_semana", dia_semana).execute() #define para qual dia da semana será plotado

data = response.data

lats = np.array([item['latitude'] for item in data])
lons = np.array([item['longitude'] for item in data])
temps = np.array([item['temperatura_max'] for item in data])
...
#plota a interpolação das temperaturas
contour = ax.contourf(
...
plt.title(f"Temperatura Máxima - Sul do Brasil ({dia_semana})")
plt.show()