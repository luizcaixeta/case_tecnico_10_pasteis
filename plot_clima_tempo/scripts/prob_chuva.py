...
#retorna as colunas cidade, latitude, longitude e prob_chuva_max
response = supabase.table('clima_cidades_sul').select(
    "cidade, latitude, longitude, prob_chuva_max"
...
temps = np.array([item['prob_chuva_max'] for item in data])
...
#interpola os dados de prob_chuva_max sobre a grade criada
grid_temp = griddata(
    (lons, lats), temps,
    (grid_lon, grid_lat),
    method='cubic'
...
#plota a interpolação das prob_chuva
contour = ax.contourf(
    grid_lon, grid_lat, masked_temp,
    levels=20, cmap=plt.cm.get_cmap('tab20c').reversed(), transform=ccrs.PlateCarree()
)

#define o título do gráfico
plt.colorbar(contour, ax=ax, label='Probabilidade de chuva (%)')
plt.title(f"Probabilidade máxima de chuva - Sul do Brasil ({dia_semana})")
plt.show()